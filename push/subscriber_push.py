...
def create_push_subscription_if_not_exists():
    ...
    try:
        ...
        print(
            f"Pushサブスクリプション {subscription_path} を {topic_path} に作成しました。"
        )
...
if __name__ == "__main__":
    print(
        f"トピック {topic_path} に対するPushサブスクリプションを作成または確認します..."
    )
    ...
    print(
        "2. `python push_endpoint_server.py` を実行してPushサーバーを起動してください。"
    )
    print(
        "3. `python publisher.py` を実行してメッセージを送信すると、Pushサーバーのログにメッセージが表示されます。"
    )
